...
class TaskProvider extends ChangeNotifier {
  ...
  int _currentFocusLevel = 3;

  bool get loading => _loading;
  String? get error => _error;
  UserProfile? get profile => _profile;
  List<FocusTask> get tasks => List.unmodifiable(_tasks);
  int get currentFocusLevel => _currentFocusLevel;

  List<FocusTask> get recommendedTasks {
    final candidates = _tasks
        .where((t) => !t.done && t.requiredEnergy <= _currentFocusLevel)
        .toList();
    candidates.sort((a, b) {
  ...
  List<FocusTask> get completedTasks => _tasks.where((t) => t.done).toList();
  ...
  void setCurrentFocusLevel(int value) {
    _currentFocusLevel = value.clamp(1, 5);
    notifyListeners();
  }
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      _currentFocusLevel = _profile?.defaultFocusLevel ?? 3;
  ...
  Future<void> save(FocusTask task) async {
    final userId = authProvider.userId;
    if (userId == null) {
      _error = '로그인이 필요합니다.';
      notifyListeners();
  ...
  Future<void> toggleDone(FocusTask task, bool done) async {
  ...
  Future<void> updateSettings({
    required int defaultFocusLevel,
    ...
    final updated = current.copyWith(
      defaultFocusLevel: defaultFocusLevel,
    ...
    try {
      ...
      _profile = updated;
      _currentFocusLevel = updated.defaultFocusLevel;
      notifyListeners();
    } catch (e) {
      _error = e.toString().replaceFirst('Exception: ', '');
      notifyListeners();
