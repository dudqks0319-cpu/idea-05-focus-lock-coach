...
class SupabaseService {
  ...
  final Map<String, List<FocusTask>> _localTasks = {};
  ...
  Future<UserProfile> fetchProfile(String userId) async {
    if (useSupabase) {
      final data = await Supabase.instance.client
          .from('profiles')
          .select('id, default_focus_level, daily_focus_minutes, notifications_enabled')
          .eq('id', userId)
          .maybeSingle();
  ...
  Future<void> updateProfile(UserProfile profile) async {
    if (useSupabase) {
      await Supabase.instance.client.from('profiles').update({
        'default_focus_level': profile.defaultFocusLevel,
  ...
  Future<List<FocusTask>> fetchTasks(String userId) async {
    if (useSupabase) {
      final rows = await Supabase.instance.client
          .from('focus_tasks')
          .select('id, title, focus_intensity, estimated_minutes, status, notes, created_at, updated_at')
          .eq('user_id', userId)
          .order('created_at', ascending: false);

      return (rows as List)
          .map((row) => FocusTask.fromMap(Map<String, dynamic>.from(row as Map)))
          .toList();
    }

    await ensureProfile(userId);
    return List<FocusTask>.from(_localTasks[userId] ?? []);
  }

  Future<void> saveTask({required String userId, required FocusTask task}) async {
    if (useSupabase) {
      await Supabase.instance.client.from('focus_tasks').upsert(task.toInsertMap(userId), onConflict: 'id');
      return;
    }
  ...
  Future<void> deleteTask({required String userId, required String taskId}) async {
    if (useSupabase) {
      await Supabase.instance.client.from('focus_tasks').delete().eq('id', taskId).eq('user_id', userId);
      return;
    }
